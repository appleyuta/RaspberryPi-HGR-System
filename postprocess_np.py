...
def yolo3_head(predictions, anchors, num_classes, input_dims):
    ...
    for i, prediction in enumerate(predictions):
        results.append(yolo_head(prediction, anchors[i*3:i*3+3], num_classes, input_dims, use_softmax=False))

    return np.concatenate(results, axis=1)


def yolo3_postprocess_np(yolo_outputs, image_shape, anchors, num_classes, model_image_size, max_boxes=100, confidence=0.1, iou_threshold=0.5):
    ...
    return center, boxes, classes, scores
